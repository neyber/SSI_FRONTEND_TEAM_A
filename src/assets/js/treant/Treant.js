    ...
    TreeNode.prototype.buildNodeFromText = function (node) {
        ...
        if (this.image) {
          ...
          if(this.image['favicon']){
            ...
            favicon.setAttribute('src',this.image['favicon']);
            node.appendChild(favicon);
          }

          if(this.image['finding']){
            atag = document.createElement('a');
            findingContainer = document.createElement('span');
            badge = document.createElement('span');
            badge.setAttribute('class','badge');
            ifinding = document.createElement('i');
            ifinding.setAttribute('class','flaticon flaticon flaticon-pen');
            findingContainer.append(badge);
            findingContainer.append(ifinding);
            atag.append(findingContainer);
            node.appendChild(atag);
          }

          if(this.image['snapshot']){
            atag = document.createElement('a');
            snapshotContainer = document.createElement('span');
            badge = document.createElement('span');
            badge.setAttribute('class','badge');
            isnapshot = document.createElement('i');
            isnapshot.setAttribute('class','fa fa-camera fa-2x');
            snapshotContainer.append(badge);
            snapshotContainer.append(isnapshot);
            atag.append(snapshotContainer);
            node.appendChild(atag);
          }
          //btn close
          atagClose = document.createElement('a');
          atagClose.setAttribute('class','remove-node');
          iclose = document.createElement('span');
          iclose.setAttribute('class','icon-close-sm');
          atagClose.append(iclose);
          node.appendChild(atagClose);


        }else{
          /* if(node.nodeStructure.type === 'revisor') {
             console.log('add default logo');
          }
        aFavTag = document.createElement('a');
          iFavIcon = document.createElement('span');
          iFavIcon.setAttribute('class','fa fa-dribbble fa-2');
          aFavTag.append(iFavIcon);
          node.appendChild(aFavTag);*/
        }
